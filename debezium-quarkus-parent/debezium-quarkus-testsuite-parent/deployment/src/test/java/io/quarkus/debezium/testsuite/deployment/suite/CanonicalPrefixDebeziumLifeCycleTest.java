...
/**
 * Verifies the canonical {@code debezium.*} configuration prefix.
 *
 * Existing test-suite resources still exercise the legacy {@code quarkus.debezium.*} prefix to ensure backward
 * compatibility. This test boots the extension using the canonical prefix only.
 */
@Tag(SuiteTags.DEFAULT)
public class CanonicalPrefixDebeziumLifeCycleTest {
    ...
    static final QuarkusUnitTest application = new QuarkusUnitTest()
            ...
            .withConfigurationResource("debezium-testsuite.properties");

    @Test
    @DisplayName("debezium should start with canonical config prefix")
    void shouldStartWithCanonicalPrefix() {
